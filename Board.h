...
public:
	...
	initial number of beans per hole. 
	If nHoles is not positive, act as if it were 1; if nInitialBeansPerHole is negative, act as if it were 0.
	*/

	Board(const Board& other); // Newly added: Copy Constructor

	Board& operator = (const Board& other);// Newly added: Assignment Operator
	
	~Board(); // Newly added: Destructor
	...
	but skipping s's opponent's pot. The parameters endSide and endHole are set to the side and hole where 
	...
	*/
	...
	it will return true after setting the number of beans in the indicated hole or pot to the value of 
	...
	None of your code that implements the member functions of 
	any class is allowed to call this function directly or indirectly. (We'll show an example of its use below.)
	*/

private:

	int m_NumOfHoles; //indicated number of holes per side
	int m_InitialBeans; //initial number of beans per hole
	int m_AllHoles; //indicated number of all the holes (including the pot) of a side

	
	std::vector<Hole*> m_SouthHole; // Vector of South Holes (pointers)
	std::vector<Hole*> m_NorthHole; // Vector of North Holes (pointers)


	//void displayBoard() const; // Display the board (in the same format as in Game)
